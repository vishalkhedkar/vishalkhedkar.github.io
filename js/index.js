function calc() {
  ...
  var gradep = [gradePoints(java), gradePoints(dbms), gradePoints(or), gradePoints(ssd), gradePoints(ai), gradePoints(javal), gradePoints(dbl), gradePoints(uml), gradePoints(mp) ];
  
  var total_credits = 4*gradep[0] + 4*gradep[1] + 4*gradep[2] + 4*gradep[3] + 3*gradep[4] + 2*gradep[5] + 2*gradep[6] + 2*gradep[7] + 1*gradep[8] ;
  
  var gpa = parseFloat((total_credits/26).toPrecision(4));
  document.getElementById("res").innerHTML = "<h2>Your GPA is: "+ gpa + "</h2>";
